...
export function ContactsPage() {
  ...
  const columns = [
    ...
    { title: 'Thao tác', fixed: 'right' as const, width: 170, render: (_: unknown, row: ContactMessageDto) => <Space><Button size="small" icon={<EyeOutlined />} onClick={() => view(row)} /><Button size="small" icon={<CheckCircleOutlined />} onClick={() => action.mutate({ id: row.id, type: 'replied' })} /><Button size="small" icon={<InboxOutlined />} onClick={() => action.mutate({ id: row.id, type: 'archive' })} /><Popconfirm title="Xóa vĩnh viễn tin nhắn này?" onConfirm={() => action.mutate({ id: row.id, type: 'delete' })}><Button danger size="small" icon={<DeleteOutlined />} /></Popconfirm></Space> }
  ];
  return <>
    ...
    <Drawer open={Boolean(selected)} title={selected?.subject ?? 'Chi tiết tin nhắn'} width={640} onClose={() => setSelected(null)} extra={selected ? <Button type="primary" icon={<CheckCircleOutlined />} onClick={() => action.mutate({ id: selected.id, type: 'replied' })}>Đã phản hồi</Button> : null}>
